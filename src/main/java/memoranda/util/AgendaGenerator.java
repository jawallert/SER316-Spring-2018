...
public class AgendaGenerator {
	...
	static String generateTasksInfo(IProject p, CalendarDate date, Collection expandedTasks) {    	    	
		...
		if (tasks.size() == 0) {
			...
			for (Iterator i = tasks.iterator(); i.hasNext();) {
				...
				//                	}	
				//        		}
				// ignore if it's a sub-iTask, iterate over ROOT tasks here only
	...
	private static String expandRecursively(IProject p,CalendarDate date, ITaskList tl,ITask t, Collection expandedTasks, int level) {
		Util.debug("Expanding iTask " + t.getText() + " level " + level);
	...
	private static String renderTask(IProject p, CalendarDate date, ITaskList tl, ITask t, int level, Collection expandedTasks) {
		...
		//		}
		//		Util.debug("Spacing for iTask " + t.getText() + " is " + spacing);
...
=======*/
	...
	static String generateEventsInfo(CalendarDate date) {
		...
		for (Iterator i = v.iterator(); i.hasNext();) {
			...
			String iurl =
					...
					.AppFrame
					.class
					.getResource("/main/resources/ui/agenda/spacer.gif")
					.toExternalForm();
			if (date.equals(CalendarDate.today())) {
				...
				if ((EventsScheduler.isEventScheduled())
					...
					iurl =
							...
							.AppFrame
							.class
							.getResource("/main/resources/ui/agenda/arrow.gif")
							.toExternalForm();
				}
			}
			String icon =
	...
	static String generateStickers(CalendarDate date) {
		String iurl =
				...
				.AppFrame
				.class
				.getResource("/main/resources/ui/agenda/addsticker.gif")
				.toExternalForm();
		String iurl2 =
				main.java.memoranda.ui
				.AppFrame
				.class
				.getResource("/main/resources/ui/agenda/removesticker.gif")
				.toExternalForm();
    ...
    public static class TaskSorter {
        ...
        public static long calcTaskRate(ITask t, CalendarDate d) {
            /*
	 * A "ITask rate" is an informal index of importance of the iTask
	 ...
	 * /
        ...
        static long getRate(Object iTask) {
            ITask t = (ITask)iTask;
            switch (mode) {
